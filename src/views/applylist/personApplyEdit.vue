          ...
          <el-row :gutter="20">
            <el-col :span="24">
              <div class="grid-content bg-purple" style="display: flex;justify-content: center;padding-top: 20px;">
                <el-button v-show="editable"  type="primary" @click="onSubmitApply">修改</el-button>
              </div>
            </el-col>
          </el-row>
  ...
  data() {
    return {
      ...
      personformrules: {
        ...
        tel: [{ required: true, message: '请输入电话', trigger: 'blur' },{validator: checkPhone, message: '请输入正确格式的手机号', trigger: 'blur'}],
    ...
    getBeianApplyInfo() {
      ...
      getBeianApplyInfo({applyid:this.$route.query.applyid}).then(response => {
        ...
        if(response.code==200){
          ...
          this.applyForm.audittime = response.datas.applyinfo.audittime
