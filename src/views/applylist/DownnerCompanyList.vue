<template>
  <div class="app-container">
    <el-row>
      <el-button type="primary" @click="createNewUpperCompany">新增下游公司</el-button>
...
export default {
  ...
  methods: {
    ...
    createNewUpperCompany(){
        this.$router.push('/apply/DownnerCompanyEdit')
    },
    getList() {
    ...
    updateCompanyInfo(row) {
      this.$router.push({
        path: "/apply/updateDownnerCompanyInfo",
    ...
    delCompanyInfo(row, status) {
      this.$confirm("此操作将永久删除单位备案信息, 是否继续?", "提示", {
        ...
        .then(() => {
          delDownnerCompany(row.id).then(response => {
            console.log(response)
