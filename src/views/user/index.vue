<template>
  <div class="app-container">
    ...
    <el-form-item label="">
      <el-input @keyup.enter.native="handleFilter" class="filter-item" :placeholder="$t('user.title')" v-model="listQuery.title">
      </el-input>
    </el-form-item>

    <el-form-item label="">
      <el-select @change="handleFilter"  clearable class="filter-item" v-model="listQuery.type" :placeholder="$t('user.type')">
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
          <span class="link-type">{{transformString(scope.row.admin_gid)}}</span>
        </template>
    </el-table-column>


    <el-table-column
      prop="admin_login_num"
      label="登录次数"
      >
    </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="70px" style='width: 400px; margin-left:50px;'>
        <el-form-item :label="$t('user.type')" prop="admin_gid">
          <el-select class="filter-item" v-model="temp.admin_gid" placeholder="Please select">
            <el-option v-for="item in  calendarTypeOptions" :key="item.key" :label="item.display_name" :value="item.key">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item v-if="temp.admin_gid === '2'" :label="$t('user.area')" prop="checkedCities">
          <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange">全选</el-checkbox>
          <div style="margin: 15px 0;"></div>
          <el-checkbox-group v-model="temp.checkedCities">
            <el-checkbox v-for="city in cities" :label="city.id" :key="city.value">{{city.value}}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <el-form-item :label="$t('user.title')" prop="admin_name">
          <el-input v-model="temp.admin_name"></el-input>
        </el-form-item>
        <el-form-item v-if="dialogStatus !== 'update'" :label="$t('user.password')" prop="password">
...
<script>
import { fetchUserList, fetchGroupList, fetchPv, createUser, deleteUser, updateUser } from '@/api/article'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        admin_name: [{ required: true, message: 'title is required', trigger: 'blur' }],
  ...
  methods: {
    getList() {
      ...
      fetchUserList(this.listQuery).then(response => {
        ...
        this.list = response.datas.adminList
    ...
    handleModifyStatus(row, status) {
      ...
      this.$confirm('此操作将永久删除该用户, 是否继续?', '提示', {
          ...
          deleteUser(row.admin_id).then(response => {
            this.$message({
              message: '操作成功',
              type: 'success'
            })
    ...
    createData() {
      ...
      vm.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createUser(vm.temp).then((response) => {
    ...
    handleUpdate(row) {
      ...
      const areas = this.temp.areaInfo.map((item, index) => 
        item.GroupId
      )
      this.temp.checkedCities = areas
